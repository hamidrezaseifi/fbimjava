...
public class UserController {
	...
	public EncryptedContentEdo getCompanyUsers(@PathVariable final Long companyid) throws Exception {
		...
		encrypedEdo.setContentObject(new FBCollectionEdo<>(UserFull.toEdoList(users)),
	...
	public EncryptedContentEdo authenticateUser(@RequestBody(required = true) final EncryptedContentEdo encrypedEdo)
			throws Exception {
		UserLoginEdo userLoginEdo = null;
		try {
			userLoginEdo = encrypedEdo.getObjectContent(UserLoginEdo.class,
					mappingJackson2HttpMessageConverter.getObjectMapper());
		} catch (final Exception ex) {
			throw new FbCustomeException("Invalid Authentication Data", ex);
		}

		final UserFull authUser = userHandler.authenticateUser(userLoginEdo.getCompanyIdent(),
				userLoginEdo.getUsername(), userLoginEdo.getPassword());
	...
	public EncryptedContentEdo saveUser(@RequestBody(required = true) final EncryptedContentEdo encrypedEdo)
			throws Exception {
	...
	@FbCoreRequestPostDataMapping(value = FbRestPaths.Core.USER_DELETE_BASE)
	public void deleteUser(@RequestBody(required = true) final EncryptedContentEdo encrypedEdo) throws Exception {

		final UserFullEdo userEdo = encrypedEdo.getObjectContent(UserFullEdo.class,
				mappingJackson2HttpMessageConverter.getObjectMapper());
		final boolean res = userHandler.removeUser(UserFull.fromEdo(userEdo));
	...
	public void setUserPassword(@RequestBody(required = true) final EncryptedContentEdo encrypedEdo) throws Exception {
		...
		final UserPasswordEdo edo = encrypedEdo.getObjectContent(UserPasswordEdo.class,
				mappingJackson2HttpMessageConverter.getObjectMapper());
		...
		if (!userHandler.setUserPassword(user, edo.getPassword())) {
			...
			throw new FBCustomizedException(EExceptionType.SaveError.name(), "Error in save customer!",
					EModule.CORE.getModuleName());
		}
	}

	@FbCoreRequestGetDataMapping(value = FbRestPaths.Core.USER_READ_BASE)
	public EncryptedContentEdo readUser(@RequestBody(required = true) @PathVariable final Long userid)
			throws Exception {
