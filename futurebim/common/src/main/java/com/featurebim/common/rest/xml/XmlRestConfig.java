package com.featurebim.common.rest.xml;
...
public class XmlRestConfig {
   ...
   *
   * @lazy i marked this lazy b/c ResTemplate is not needed for all unit tests but this config is; in fact those unit test often dont have
   *       the dependant {@link MappingJackson2XmlHttpMessageConverter} in its spring context causing the context startup to fail.
   */
  @Lazy
  @Bean
  public RestTemplate jaxbRestTemplate(final MappingJackson2XmlHttpMessageConverter converter) {
    ...
    for (int i = 0; i < messageConverters.size(); i++) {
      /*
       * if (messageConverters.get(i).canRead(CustomerEdo.class, MediaType.APPLICATION_XML)) { messageConverters.set(i, converter); }
       */
    }
