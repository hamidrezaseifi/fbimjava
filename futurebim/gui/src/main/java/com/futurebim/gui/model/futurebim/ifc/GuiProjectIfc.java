...
public class GuiProjectIfc {
  ...
  public GuiProjectIfc(final ProjectIfcEdo edo) {
    ...
    for(final IfcPresentationlayerEdo item:edo.getLayers()){
      addLayer(new GuiIfcPresentationlayer(item));
    }

    for(final IfcPropertyEdo item:edo.getProperties()){
      addProperty(new GuiIfcProperty(item));
    }
