...
public class UserHandler implements IUserHandler {
  ...
  public List<GuiUserFull> listCompanyUsers(final Long companyId) {
    ...
    final EncryptedContentEdo encrypedResEdo = restTemplateCall.callRestGet(coreAccessConfig.getUserCompanyUserList(),
        EModule.CORE,
        EncryptedContentEdo.class,
        true, companyId);
    
    try {
      final UserFullCollectionEdo edoCollection = encrypedResEdo.getObjectContent(UserFullCollectionEdo.class,
          mappingJackson2HttpMessageConverter.getObjectMapper());
      return GuiUserFull.fromEdoList(edoCollection.getUsers());
    }
    catch (final Exception e) {
      return null;
    }
